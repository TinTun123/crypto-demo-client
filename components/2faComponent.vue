<template>
        <div class="rounded-[8px] bg-white dark:bg-[var(--secondary-dark)] p-[32px] w-full transition-colors">
            ...
            <span class="text-[14] text-[#444444] dark:text-[var(--text-dark)] font-[400]">An authencation code was sent to {{ auth.userEmail }}</span>
            <div class="flex flex-col gap-y-3 mb-[15px]">
                <label :class="[themestore.isgrad ? 'text-[var(--theme-from)]' : 'text-[var(--theme-color)]']" for="code" class="text-[15px] fron-[600] ">Authentication Code</label>
                <input v-model="form.code" :class="[error.type === 'code' ? 'border border-red-300' : 'border-[#eeeeee] dark:border-none']" class="bg-white w-full dark:bg-[var(--teriary-dark)] text-[#444444] dark:text-[var(--text-dark)] dark:shadow-none rounded-[4px] shadow-input-sha border text-[15px] p-[15px] placeholder-gray-500 focus:outline-none" placeholder="Code"/>
                <span v-if="error.type === 'code'" class="text-[12px] text-red-500 font-[500]">{{ error.msg }}</span>
            </div>

            <div class="flex justify-between gap-x-4">
                <button @click.stop="changeTabTo0" class="py-[12px] px-[40px] mb-[15px] rounded-[6px] w-full text-[14px] font-[800] text-white bg-[#444444] dark:[var(--teriary-dark)]">BACK</button>
                
                <button @click.stop="send" class="py-[12px] px-[40px] flex justify-center items-center gap-x-3 mb-[15px] rounded-[6px] w-full text-[14px] font-[800] text-white bg-[var(--theme-color)]">
                    <span>
...
async function send() {
    ...
    if (!form.value.code) {
        ...
        error.value.msg = 'Please enter verification code';
        isLoading.value = false;
        return;
    }


    const {data : loginData, error : loginErr, status } = await auth.verify2fa(form.value);
    isLoading.value = false;
